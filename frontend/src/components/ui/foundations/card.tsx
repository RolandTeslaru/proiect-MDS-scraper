...
function Root({
  ...
  return (
    <div
      ...
      data-size={size}
      className={cn(`
        ring-foreground/15 bg-card/70 text-card-foreground gap-4 overflow-hidden 
        ...
        *:[img:last-child]:rounded-b-lg group/card flex flex-col`, className)}
      {...props}
    />
  )
}
...
function Footer({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("bg-muted/50 rounded-b-lg border-t p-4 group-data-[size=sm]/card:p-3 flex items-center", className)}
      {...props}
    />
  )
}
